...
then
    ...
    command -v notify-send >/dev/null && notify-send -h int:transient:1 "S/PDIF: $state" -t 500
    exit 0
fi
...
command -v notify-send >/dev/null && notify-send -h int:transient:1 "Volume: $new_vol" -t 500
