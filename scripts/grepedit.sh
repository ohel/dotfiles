...
rg -S -uu $1 2>/dev/null || ag $1 2>/dev/null || grep -n -R -i --color=auto $1 2>/dev/null
...
[ "$rg_exe" ] && cmd="rg -S -uu -l --no-heading --with-filename --color never --column $1 . $greps | xargs $editor"
