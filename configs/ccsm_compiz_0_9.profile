[workarounds]
s0_keep_minimized_windows = true
s0_ooo_menu_fix = false
s0_java_fix = false
s0_java_taskbar_fix = false
s0_aiglx_fragment_fix = false

[staticswitcher]
s0_background_color = #333333d9

[thumbnail]
s0_fade_speed = 0.000000
s0_border = 2
s0_thumb_color = #0000007f
s0_window_like = false
s0_title_enabled = false
s0_font_bold = false
s0_font_background_color = #0000007f

[vpswitch]
s0_next_button = Button5
s0_prev_button = Button4

[screenshot]
s0_selection_outline_color = #2f2f4f9f
s0_selection_fill_color = #2f2f4f4f

[switcher]
s0_next_all_key = Disabled
s0_prev_all_key = Disabled
s0_speed = 4.500000
s0_timestep = 0.100000
s0_brightness = 100
s0_opacity = 100
s0_zoom = 0.000000
s0_auto_rotate = true
s0_background_color = #000000ff

[scaleaddon]
s0_close_button = Disabled
s0_zoom_button = Disabled
s0_window_title = 2
s0_title_size = 20
s0_border_size = 5
s0_highlight_color = #ffffff1a
s0_layout_mode = 1
s0_natural_precision = 10.000000
s0_exit_after_pull = true

[ezoom]
s0_zoom_in_button = <Super>Button4
s0_zoom_in_key = <Super>KP_Add
s0_zoom_out_button = <Super>Button5
s0_zoom_out_key = <Super>KP_Subtract
s0_zoom_box_outline_color = #2f2f4f9f
s0_zoom_box_fill_color = #2f2f2f4f
s0_zoom_specific_1_key = <Super>KP_Insert
s0_pan_left_key = <Super>KP_Left
s0_pan_right_key = <Super>KP_Right
s0_pan_up_key = <Super>KP_Up
s0_pan_down_key = <Super>KP_Down
s0_fit_to_window_key = <Super>KP_Begin
s0_zoom_mode = 1
s0_pan_factor = 0.200000
s0_speed = 30.000000
s0_timestep = 0.100000

[fade]
s0_fade_mode = 1
s0_fade_speed = 1.000000
s0_window_match = any
s0_unresponsive_brightness = 75

[annotate]
s0_initiate_line_button = Disabled
s0_initiate_rectangle_button = Disabled
s0_initiate_ellipse_button = Disabled
s0_erase_button = Disabled
s0_clear_button = <Alt><Super>Button3
s0_clear_key = Disabled
s0_stroke_color = #ff00ffff
s0_stroke_width = 5.000000

[showmouse]
s0_color = #ffdf3fff

[winrules]
s0_no_focus_match = class=Conky

[wall]
s0_thumb_highlight_gradient_shadow_color = #dfdfdfff
s0_arrow_base_color = #e6e6e6d9
s0_arrow_shadow_color = #dcdcdcd9

[core]
s0_active_plugins = core;crashhandler;composite;opengl;compiztoolbox;decor;neg;mousepoll;imgpng;ezoom;text;place;annotate;imgjpeg;imgsvg;session;cube;regex;move;grid;resize;put;scale;thumbnail;switcher;scaleaddon;winrules;rotate;resizeinfo;
s0_audible_bell = false
s0_outputs = 1920x1080+0+0;
s0_focus_prevention_level = 0
s0_focus_prevention_match = any
s0_close_window_key = Disabled
s0_raise_window_button = Disabled
s0_lower_window_button = Disabled
s0_minimize_window_key = Disabled
s0_maximize_window_key = Disabled
s0_unmaximize_or_minimize_window_key = Disabled
s0_window_menu_button = Disabled
s0_toggle_window_maximized_key = Disabled
s0_toggle_window_shaded_key = Disabled
s0_hsize = 2

[extrawm]
s0_toggle_always_on_top_key = <Alt><Super>t

[opengl]
s0_texture_filter = 2
s0_lighting = true
s0_texture_compression = true

[firepaint]
s0_fire_color = #ff3305ff

[crashhandler]
s0_start_wm = true
s0_wm_cmd = xfwm4 --replace

[rotate]
s0_edge_flip_window = false
s0_edge_flip_dnd = false
s0_flip_time = 100
s0_speed = 4.000000
s0_timestep = 0.100000
s0_initiate_button = Disabled
s0_rotate_left_key = Disabled
s0_rotate_right_key = <Super>Tab
s0_rotate_left_window_key = Disabled
s0_rotate_right_window_key = <Shift><Super>Tab

[expo]
s0_ground_color1 = #b3b3b3cc
s0_ground_color2 = #b3b3b300

[cube]
s0_unfold_key = Disabled
s0_speed = 2.500000

[resizeinfo]
s0_fade_time = 100
s0_resizeinfo_font_bold = false
s0_gradient_1 = #ffffff80
s0_gradient_2 = #ffffff80
s0_gradient_3 = #ffffff80
s0_outline_color = #000000ff

[move]
s0_initiate_key = <Super>Return
s0_opacity = 90
s0_key_move_inc = 50
s0_snapoff_semimaximized = false
s0_snapback_semimaximized = false

[cubeaddon]
s0_reflection = false
s0_ground_color1 = #b3b3b3cc
s0_ground_color2 = #b3b3b300
s0_unfold_deformation = false
s0_deform_caps = false
s0_draw_top = false
s0_draw_bottom = false
s0_top_scale = false
s0_bottom_scale = false
s0_top_aspect = false
s0_bottom_aspect = false
s0_top_clamp = false
s0_bottom_clamp = false

[composite]
s0_unredirect_match = (any) & !(class=mpv) & !(class=MPlayer) & !(class=Vlc)

[grid]
s0_put_center_key = <Alt><Super>o
s0_put_left_key = <Alt><Super>h
s0_put_right_key = <Alt><Super>l
s0_put_top_key = <Alt><Super>k
s0_put_bottom_key = <Alt><Super>j
s0_put_topleft_key = <Alt><Super>KP_Home
s0_put_topright_key = <Alt><Super>KP_Prior
s0_put_bottomleft_key = <Alt><Super>KP_End
s0_put_bottomright_key = <Alt><Super>KP_Next
s0_put_maximize_key = Disabled
s0_put_restore_key = <Alt><Super>n
s0_left_maximize = Disabled
s0_right_maximize = Disabled
s0_top_left_corner_action = 7
s0_top_edge_action = 8
s0_top_right_corner_action = 9
s0_bottom_left_corner_action = 1
s0_bottom_edge_action = 2
s0_bottom_right_corner_action = 3
s0_snapback_windows = false
s0_cycle_sizes = true
s0_snapoff_threshold = 0
s0_animation_duration = 100

[resize]
s0_initiate_button = <Alt>Button3
s0_initiate_key = <Alt><Super>Return
s0_maximize_vertically = false
s0_border_color = #2f2f4f9f
s0_fill_color = #2f2f4f4f

[freewins]
s0_circle_color = #54befb80
s0_line_color = #1800ffff
s0_cross_line_color = #1800ffff

[put]
s0_put_center_key = <Super>n
s0_put_left_key = <Super>h
s0_put_right_key = <Super>l
s0_put_top_key = <Super>k
s0_put_bottom_key = <Super>j
s0_put_topleft_key = Disabled
s0_put_topright_key = Disabled
s0_put_bottomleft_key = Disabled
s0_put_bottomright_key = Disabled
s0_put_restore_key = Disabled
s0_put_pointer_key = Disabled
s0_put_next_output_key = <Super>o
s0_pad_left = 10
s0_pad_right = 10
s0_pad_top = 10
s0_pad_bottom = 10
s0_avoid_offscreen = true
s0_speed = 12.500000
s0_timestep = 0.100000

[animationaddon]
s0_beam_color = #7f7f7fff
s0_fire_color = #ff3305ff

[scale]
s0_spacing = 0
s0_speed = 10.000000
s0_darken_back = false
s0_opacity = 75
s0_hover_time = 500
s0_dnd_distance = 5
s0_multioutput_mode = 0
s0_initiate_key = <Super>space
s0_initiate_all_key = Disabled

[decor]
s0_active_shadow_radius = 6.000000
s0_active_shadow_opacity = 6.000000
s0_active_shadow_color = #ff0000ff
s0_active_shadow_x_offset = 0
s0_active_shadow_y_offset = 0
s0_inactive_shadow_radius = 6.000000
s0_inactive_shadow_opacity = 1.000000
s0_inactive_shadow_color = #00000080
s0_inactive_shadow_x_offset = 0
s0_inactive_shadow_y_offset = 0
s0_command = /opt/programs/compiz/bin/gtk-window-decorator --metacity-theme "metacity_panther"
s0_mipmap = true
s0_decoration_match = !any
s0_shadow_match = (any) & !(class=Remmina)

[shift]
s0_ground_color1 = #b3b3b3cc
s0_ground_color2 = #b3b3b300

[neg]
s0_window_toggle_key = <Super>f
s0_screen_toggle_key = Disabled

